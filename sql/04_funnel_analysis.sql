...
SELECT
  ...
  trial_users,
  ROUND(trial_users * 100.0 / NULLIF(verify_users,0), 2) AS  activation_rate_pct,                    -- KPI: Activation Rate (users who start trial after verification)
  subscribed_users,
...
SELECT
  ...
  ROUND((PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY hours_signup_to_verify))::numeric, 2) AS med_signup_to_verify_hrs,   -- KPI: Verification Delay (median hours from signup to email verification)
...
-- KPI: Early Engagement Gap
-- Users who signed up but never triggered feature_use (activation risk segment)
...
SELECT
  COUNT(*) AS users_with_no_early_engagement
FROM user_steps
...
SELECT
  ROUND(
    COUNT(DISTINCT CASE WHEN trial_time IS NOT NULL THEN user_id END) * 100.0 /
    NULLIF(COUNT(DISTINCT CASE WHEN verify_time IS NOT NULL THEN user_id END),0),
    2
  ) AS activation_rate_pct,
  ROUND(
    PERCENTILE_CONT(0.5)
      WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (verify_time - signup_time))/3600.0)::numeric,
    2
  ) AS verification_delay_hrs
FROM funnel;
